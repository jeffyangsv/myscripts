#/bin/bash
##################################################
#Name:        prometheus_node_exporter.sh
#Version:     v0.9.0
...
App=node_exporter-0.14.0-rc.1.linux-amd64
...
AppProg=$AppInstallDir/node_exporter
...
fpid()
{
    AppMasterPid=$(ps ax | grep "node_exporter" | grep -v "grep" | awk '{print $1}' 2> /dev/null)
}
...
{ 
    ...
    cp -vf $AppConf $AppConfDir/  &>/dev/null
}
