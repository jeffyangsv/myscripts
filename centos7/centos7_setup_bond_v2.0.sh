...
fsetbond(){
    if [ $# -eq 2 ];then
        ...
        echo "USAGE: $0 setbond [1,4,6] ${BOND_IP_BASE}.HOST"
        exit 3    
    fi

    if [ $( grep "options bond0 miimon=100 mode=${MODE_NUM}" /etc/modprobe.conf | wc -l) -eq 0 ];then
       echo "alias bond0 bonding
options bond0 miimon=100 mode=${MODE_NUM}" > /etc/modprobe.conf
    else
        echo "modprobe.conf文件已设置"
    fi
...
#        echo "bond已设置,需重启服务器，请输入'reboot'" 
    ...
    if [ ${MODE_NUM} -eq 4 ];then
        sed -i "/^BONDING_OPTS=/s/BONDING_OPTS/#BONDING_OPTS/" /etc/sysconfig/network-scripts/ifcfg-bond0
        echo "BONDING_OPTS=\"mode=4 miimon=100 xmit_hash_policy\" " >> /etc/sysconfig/network-scripts/ifcfg-bond0
    elif [ ${MODE_NUM} -eq 6 ];then
        sed -i "/^BONDING_OPTS=/s/BONDING_OPTS/#BONDING_OPTS/" /etc/sysconfig/network-scripts/ifcfg-bond0
        echo "BONDING_OPTS=mode=balance-alb"   >> /etc/sysconfig/network-scripts/ifcfg-bond0
    else
        sed -i "/^BONDING_OPTS=/s/BONDING_OPTS/#BONDING_OPTS/" /etc/sysconfig/network-scripts/ifcfg-bond0
        echo "BONDING_OPTS=mode=active-backup" >> /etc/sysconfig/network-scripts/ifcfg-bond0
    fi
    echo "bond已设置,需重启服务器,请输入'reboot'" 
}
...
case "$1" in
    ...
    echo "$ScriptFile setbond  mode_num bond_ip 设置bond"
    ;;
esac
