#/bin/bash
##################################################
#Name:        prometheus_alertmanager.sh
#Version:     v0.5.1
#Create_Date: 2017-3-1
#Author:      GuoLikai(glk73748196@sina.com)
#Description: "Alertmanager源码Go语言编写,可直接启动"
##################################################

App=alertmanager-0.5.1.linux-amd64
...
fpid()
{
    AppMasterPid=$(ps ax | grep "alertmanager" | grep -v "grep" | awk '{print $1}' 2> /dev/null)
}
...
fupdate()
{
    ...
    mv $AppInstallDir/simple.yml $AppInstallDir/$AppName.yml
    if [ $? -eq 0 ]; then
...
fstart()
{
    ...
    if [ -n "$AppMasterPid" ]; then
        ...
        $AppProg -config.file=$AppConfDir/$AppName.yml  &>/dev/null &
	sleep 0.5
