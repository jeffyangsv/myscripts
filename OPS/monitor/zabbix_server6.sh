#!/bin/bash
##################################################
#Name:        zabbix_server6.sh
#Version:     v1.0
#Create_Date：2016-6-10
#Author:      GuoLikai(glk73748196@sina.com)
#Description: "Linux服务器部署zabbix软件"
##################################################
...
echo “执行此脚本前要创建数据库zabbixdb并对其进行用户授权！！！”
read -p " 请输入数据库zabbixdb授权用户: " User
read -p " 请输入数据库zabbixdb授权用户密码: " Sec
read -p " 请输入zabbixdb服务器的ip地址: " IP
read -p " 请设置zabbixdb服务器主机名: " NAME
read -p " 请输入zabbixdb压缩包完整路径: " DIR
yum -y install httpd  mysql*  php*  &>/dev/null #rhel6上操作
#yum -y install httpd mariadb*  php* &>/dev/null #rhel7上操作
