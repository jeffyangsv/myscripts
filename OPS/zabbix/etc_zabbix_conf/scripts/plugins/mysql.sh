...
mysql_socket="-S $(echo "$@"|awk '{print $NF}')"

case $1 in
  "uptime")
    $MYSQLADMIN_BIN $mysql_opts $mysql_socket status | cut -f2 -d':' | cut -f1 -d'T' | sed -e 's/^\s*//'
    ;;

  "threads")
    $MYSQLADMIN_BIN $mysql_opts $mysql_socket status | cut -f3 -d':' | cut -f1 -d'Q' | sed -e 's/^\s*//'
    ;;

  "slowqueries")
    $MYSQLADMIN_BIN $mysql_opts $mysql_socket status | cut -f5 -d':' | cut -f1 -d'O' | sed -e 's/^\s*//'
    ;;

  "questions")
    $MYSQLADMIN_BIN $mysql_opts $mysql_socket status | cut -f4 -d':' | cut -f1 -d'S' | sed -e 's/^\s*//'
    ;;

  "qps")
    $MYSQLADMIN_BIN $mysql_opts $mysql_socket status | cut -f9 -d':' | sed -e 's/^\s*//'
    ;;

  "ping")
    $MYSQLADMIN_BIN $mysql_opts $mysql_socket ping | grep alive | wc -l
    ;;

  "opens")
    $MYSQLADMIN_BIN $mysql_opts $mysql_socket status | cut -f6 -d':' | cut -f1 -d'F' | sed -e 's/^\s*//'
    ;;

  "opentables")
    $MYSQLADMIN_BIN $mysql_opts $mysql_socket status | cut -f8 -d':' | cut -f1 -d'Q' | sed -e 's/^\s*//'
    ;;

  "flushtables")
    $MYSQLADMIN_BIN $mysql_opts $mysql_socket status | cut -f7 -d':' | cut -f1 -d'O' | sed -e 's/^\s*//'
    ;;

  com_* | "bytes_received" | "bytes_sent")
    mysql_batch_query "$mysql_opts $mysql_socket" "show global status" | egrep -i "^$1\s+" | awk '{ print $2 }' | sed -e 's/^\s*//'
    ;;
  ...
  "replication" | "slave")
    if [ ! -e $MYCNF_PATH ]; then
      ...
      slave_opts="-u $slave_user --password=$slave_password -h $slave_host $mysql_socket"
    else
      slave_opts="--defaults-file=$MYCNF_PATH $mysql_socket"
    fi
...
EOF
      ...
      "checksum")
        ...
        master_opts="-u $master_user --password=$master_password -h $master_host $mysql_socket"
