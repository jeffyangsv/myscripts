...
#Description: "kibana一键部署启动脚本"
##################################################

App=kibana-5.3.2-linux-x86_64
AppName=kibana
...
AppProg=$AppOptDir/bin/kibana
AppProgCli=$AppOptDir/bin/kibana-plugin
AppConf=$AppConfDir/$AppName.yml
...
{   
	...
	sed -i "/#server.port:/cserver.port: 5601"  $AppConfDir/$AppName.yml
	sed -i "/#server.port:/cserver.port: 5601"  $AppConfDir/$AppName.yml
	sed -i "/#server.host:/cserver.host: \"0.0.0.0\""  $AppConfDir/$AppName.yml
	sed -i "/#elasticsearch.url:/celasticsearch.url: \"http://${HostIp}:9200\""  $AppConfDir/$AppName.yml
	sed -i "/#kibana.index:/ckibana.index: \".kibana\""  $AppConfDir/$AppName.yml
}
